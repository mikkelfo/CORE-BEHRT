...
class DatasetPreparer:
    ...
    def _save_sequence_lengths(self, data):
        if not data.outcomes:
            ...
            torch.save(sequence_lens, join(self.run_folder, f'sequences_lenghts_{data.mode}.pt'))
            return data
        else:
            ...
            torch.save(sequence_lens_neg, join(self.run_folder, f'sequences_lenghts_{data.mode}_neg.pt'))
            torch.save(sequence_lens_pos, join(self.run_folder, f'sequences_lenghts_{data.mode}_pos.pt'))
            return data
