...
def dynamic_padding(data: list)->dict:
    ...
    for key in data[0].keys():
        if (
            key == "target" and data[0]["target"].ndim == 3
        ):  # For hierarchical pretraining only
            padded_data[key] = torch.cat(
                [patient[key] for patient in data]
            )  # We give flat labels as we manually mask them in the loss function
        else:
