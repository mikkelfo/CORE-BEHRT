...
class TestMLMDataset(unittest.TestCase):
    ...
    def test_init(self, mock_torch_load):
        ...
        dataset = MLMDataset(mock_data_dir, mock_mode, vocabulary=mock_vocabulary)
    ...
    def test_getitem(self, mock_torch_load):
        ...
        special_token_indices = (torch.tensor(mock_torch_load.return_value['concept']) < 2).nonzero(as_tuple=True)[0]
        self.assertTrue(torch.all(patient['concept'][special_token_indices] != 0))
