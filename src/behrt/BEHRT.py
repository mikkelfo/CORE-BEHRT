...
class BehrtConfig(Bert.modeling.BertConfig):
    def __init__(self, config):
        super(BehrtConfig, self).__init__(
...
class BehrtEmbeddings(torch.nn.Module):
    ...
    def __init__(self, config):
        super(BehrtEmbeddings, self).__init__()
...
class BehrtModel(Bert.modeling.BertPreTrainedModel):
    def __init__(self, config):
        super(BehrtModel, self).__init__(config)
        self.embeddings = BehrtEmbeddings(config=config)
        self.encoder = Bert.modeling.BertEncoder(config=config)
        self.pooler = Bert.modeling.BertPooler(config)
        self.apply(self.init_bert_weights)

    def forward(self, batch, output_all_encoded_layers=True):
        # Unpack batch
...
class BehrtForMaskedLM(Bert.modeling.BertPreTrainedModel):
    def __init__(self, config):
        super(BehrtForMaskedLM, self).__init__(config)
        self.bert = BehrtModel(config)
    ...
    def forward(self, batch):
        sequence_output, _ = self.bert(batch, output_all_encoded_layers=False)
