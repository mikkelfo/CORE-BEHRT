...
class Handler:
    ...
    def handle(self, features: dict, concept_fill="[UNK]", num_fill=-100, drop=True):
        ...
        for i in range(len(features["concept"])):
            ...
            patient = self.handle_nans(
                patient, concept_fill=concept_fill, num_fill=num_fill, drop=drop
            )
    ...
    def handle_incorrect_ages(patient: dict, num_fill=-100, drop=True):
        correct_indices = [
            i for i, age in enumerate(patient["age"]) if -1 <= age <= 120
        ]
        if drop:
            for key, values in patient.items():
            ...
            for key, values in patient.items():
                patient[key] = [
                    values[i] if i in correct_indices else num_fill
    ...
    def handle_nans(patient: dict, concept_fill="[UNK]", num_fill=-100, drop=True):
        if drop:
            ...
            for key, values in patient.items():
                patient[key] = [
                    v for i, v in enumerate(values) if i not in set(nan_indices)
                ]
